...
function Connected({client}:{client:Client}) {
 ...
 async function submit(event:React.FormEvent<HTMLFormElement>){
  ...
  catch(e){setError(e instanceof Error&&e.message.startsWith('Persistence')?e.message:(client.hosted?'Sign-in failed. Please check your account and try again.':'Sign-in failed. Check your details and the local services, then try again.'))}
 ...
 return <main className="app-frame">
  ...
  {state.phase==='signed-out'?<section className="signin">
   ...
   <form onSubmit={submit}>{client.signInMethod==='email'&&<><label>Email<input required name="email" type="email" autoComplete="username"/></label>
   ...
   <button className="primary" disabled={busy}>{busy?'Signing in…':client.signInMethod==='google'?'Sign in with Google':'Sign in'}</button></form>
   {!client.hosted&&<p className="fine">Local review · synthetic accounts only</p>}
...
export default function Home(){
 ...
 useEffect(()=>{let active=true;import('../data/client').then(async c=>{await c.boot();if(active)setClient(c)}).catch(()=>setError('Unable to initialize the application. Reload to retry.'));return()=>{active=false}},[]);
